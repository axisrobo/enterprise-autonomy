...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "compliance-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\compliance-domain"
$adapterBinary = Join-Path $adapterDir "compliance-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8098 --data-file `"$dataDir\compliance-domain-data.json`"" -RedirectStandardOutput "$logDir\compliance-domain.out.log" -RedirectStandardError "$logDir\compliance-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8098/healthz")
