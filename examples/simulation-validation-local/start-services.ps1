...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "simulation-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\simulation-domain"
$adapterBinary = Join-Path $adapterDir "simulation-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8097 --data-file `"$dataDir\simulation-domain-data.json`"" -RedirectStandardOutput "$logDir\simulation-domain.out.log" -RedirectStandardError "$logDir\simulation-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8097/healthz")
