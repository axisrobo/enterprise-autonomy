...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "sandbox-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\sandbox-domain"
$adapterBinary = Join-Path $adapterDir "sandbox-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8101 --data-file `"$dataDir\sandbox-domain-data.json`"" -RedirectStandardOutput "$logDir\sandbox-domain.out.log" -RedirectStandardError "$logDir\sandbox-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8101/healthz")
