...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "process-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\process-domain"
$adapterBinary = Join-Path $adapterDir "process-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8100 --data-file `"$dataDir\process-domain-data.json`"" -RedirectStandardOutput "$logDir\process-domain.out.log" -RedirectStandardError "$logDir\process-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8100/healthz")
