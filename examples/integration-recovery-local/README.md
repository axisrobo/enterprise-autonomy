...
1. **Open** — the integration owner opens a governed outage case.
2. **Detect** — the outage and in-flight work are asserted with evidence.
3. **Preserve gate** — resume before preservation is rejected.
4. **Preserve** — the in-flight work is preserved under a durable reference.
5. **Verify gate** — resume before verification is rejected; the integration is verified.
6. **Process** — a durable recovery process is opened.
7. **Plan** — a verified recovery plan is generated (recommendation only).
8. **Resume and complete** — the work resumes, completes, and a repeat completion is rejected.
