...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "maintenance-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\maintenance-domain"
$adapterBinary = Join-Path $adapterDir "maintenance-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8095 --data-file `"$dataDir\maintenance-domain-data.json`"" -RedirectStandardOutput "$logDir\maintenance-domain.out.log" -RedirectStandardError "$logDir\maintenance-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8095/healthz")
