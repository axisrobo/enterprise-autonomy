...
```powershell
# order-fulfillment-local, procurement-local, customer-case-local, recruitment-local, predictive-maintenance-local, integration-recovery-local, simulation-validation-local, compliance-audit-local, fleet-mission-local, process-to-outcome-local
.\run-all.ps1
```
...
```powershell
.\run-all-demos.ps1                # all ten demos
