...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "recruitment-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\recruitment-domain"
$adapterBinary = Join-Path $adapterDir "recruitment-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8094 --data-file `"$dataDir\recruitment-domain-data.json`"" -RedirectStandardOutput "$logDir\recruitment-domain.out.log" -RedirectStandardError "$logDir\recruitment-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8094/healthz")
