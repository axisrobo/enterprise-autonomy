...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "procurement-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\procurement-domain"
$adapterBinary = Join-Path $adapterDir "procurement-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8092 --data-file `"$dataDir\procurement-domain-data.json`"" -RedirectStandardOutput "$logDir\procurement-domain.out.log" -RedirectStandardError "$logDir\procurement-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8092/healthz")
