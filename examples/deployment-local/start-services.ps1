...
$env:PRAXOVELA_BOUNDARIES = Join-Path $PSScriptRoot "deployment-boundaries.yaml"
...
$adapterDir = Join-Path $PSScriptRoot "..\..\adapters\deployment-domain"
$adapterBinary = Join-Path $adapterDir "deployment-domain-adapter.exe"
...
Start-Process -FilePath $adapterBinary -ArgumentList "--addr :8102 --data-file `"$dataDir\deployment-domain-data.json`"" -RedirectStandardOutput "$logDir\deployment-domain.out.log" -RedirectStandardError "$logDir\deployment-domain.err.log" -WindowStyle Hidden

$checks = @("http://localhost:10255/healthz", "http://localhost:8082/healthz", "http://localhost:8083/api/v1/health", "http://localhost:8080/ready", "http://127.0.0.1:8420/health", "http://localhost:8102/healthz")
