# Fleet-Domain Adapter — API Reference
...
`mission.status` → `running`, position set to the mission zone.
...
`mission.status` → `paused`. Errors: `400` missing fields, `409 mission_not_active`.
...
`mission.status` → `resumed` (resume/adjust) or `cancelled`. Gated on a paused mission and the mission operator.
...
`mission.status` → `completed`. Errors: `400` missing fields, `403 mission_not_active`.
...
`start` moves `planned → running`; an exception or exception-status telemetry moves to `paused`; an operator review resumes/adjusts/cancels; `complete` requires an active mission.
