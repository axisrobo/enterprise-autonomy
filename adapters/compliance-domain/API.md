# Compliance-Domain Adapter — API Reference
...
`case.status` → `evidence` once all items are collected.
...
`case.status` → `attested` when attested. Errors: `400` missing/invalid fields, `403 evidence_incomplete_attestation_requires_all_items`, `403 not_designated_attestor`.
...
Creates `package-comp-pkg-v1`; `case.status` → `released`. Gated on an `attest` decision citing the exact reference; released packages are immutable.
...
`evidence` transitions to `open → evidence` when all items are collected; an `attest` decision moves to `attested`; the package release moves to `released` and is immutable.
