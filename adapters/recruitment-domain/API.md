# Recruitment-Domain Adapter — API Reference
...
`requisition.status` → `validated`. Errors: `400` missing fields, `403 only_ta_lead_can_validate`, `409 requisition_not_draft`.
...
Creates `offer-rec-offer-v1` (status `issued`) and sets `requisition.status` → `closed`.
...
`validate` moves `draft → validated`; the first `shortlist` decision moves to `shortlisting`; the first `selection` decision to `selection`; the first `offer` decision to `offer`; the `offers` endpoint moves to `closed`.
