...
5. To release: bump `VERSION` with `.\scripts\version.ps1`, add a `CHANGELOG.md` section, commit and push — `check-release.ps1` will report `2`, `release-body-preview.ps1` shows the notes that will be published, `verify-release.ps1` runs the full release checklist (including the tag↔release consistency gate), and the [release workflow](.github/workflows/release.yml) creates the tag and GitHub Release automatically (or run `.\scripts\create-tag.ps1` and `.\scripts\create-release.ps1` manually).
