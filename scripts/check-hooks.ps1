...
try {
  ...
  if (-not $env:CI -and $hooksPath -ne ".githooks") { $failures += "core.hooksPath is '$hooksPath', expected '.githooks' (run install-hooks.ps1)" }
  if ($failures.Count -eq 0) {
    Write-Host "check-hooks: pre-commit hook is present and active." -ForegroundColor Green
    exit 0
  }
